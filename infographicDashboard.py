...
with col_graph1:
    # แก้ไขพารามิเตอร์ value_name ให้ถูกต้อง
    bar_data = pd.melt(
        df, 
        id_vars=["ชื่อลูกค้า"], 
...
max_p = df.loc[df['เงินต้น (บาท)'].idxmax()]
min_p = df.loc[df['เงินต้น (บาท)'].idxmin()]

st.markdown(f"""
* **ยอดเงินกู้สูงสุด:** **{max_p['เงินต้น (บาท)']:,.2f} บาท** ({max_p['ชื่อลูกค้า']})
* **ยอดเงินกู้น้อยที่สุด:** **{min_p['เงินต้น (บาท)']:,.2f} บาท** ({min_p['ชื่อลูกค้า']})

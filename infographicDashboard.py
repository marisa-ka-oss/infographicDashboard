...
with tab1:
    # แก้ไขแกน X ให้ใช้คอลัมน์ "ลูกค้า/สัญญา" เพื่อแสดงครบ 5 รายการ
    fig_bar = px.bar(
        df, 
        x="ลูกค้า/สัญญา", 
        y=["จำนวนเงินต้น", "ดอกเบี้ยรวม"],
        title="เปรียบเทียบสัดส่วนเงินต้นและดอกเบี้ยรวมของลูกค้าแต่ละสัญญา (แสดงครบ 5 รายการ)",
...
# ปุ่มดาวน์โหลด CSV
csv_bytes = df[["ชื่อลูกค้า", "จำนวนเงินต้น", "จำนวนปีที่ผ่อน", "ผ่อนต่อเดือน", "ยอดชำระรวม", "ดอกเบี้ยรวม"]].to_csv(index=False, encoding='utf-8-sig').encode('utf-8-sig')
st.download_button(
